...
- [x] M76: Fixed the live-login video feed rendering solid black despite a genuinely successful connection — noVNC's canvas was measured while its container was `display: none`
  ...
  - Next Actions: Deploy to production and confirm the video feed actually renders and the full flow (log in inside the embedded browser → "I'm logged in" → session saved) completes end-to-end.
