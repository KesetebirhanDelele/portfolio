...
- [x] M75: Fixed the Colaberry live-login WebSocket using the wrong protocol/host in production, once HTTPS made the bug visible
  ...
  - Next Actions: Deploy to production and confirm the live-login stream actually connects and shows the embedded browser (steps 1→2→3→4 in the modal), not just that the mixed-content error is gone.
